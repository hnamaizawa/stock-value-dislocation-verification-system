...
- ◎☆後の平均リターン縦棒を、期間×平均リターン×プラス率×確定件数の推移バブル図へ置換。
- 条件別平均リターン縦棒を、平均リターン×プラス率×確定件数のバブル図へ置換。
- ゼロ基準線、50%プラス率基準線、詳細ホバーを追加。
- 重複していた条件バブルを「条件別グラフの期間」の位置へ統合。
- 株価チャート用の説明文が実績分析末尾へ誤表示される配置を修正。


## v0.6.68

- 履歴・実績検証へ、条件×評価期間の平均リターンヒートマップを追加。
- 平均リターン×プラス率×確定件数の条件別バブル図を追加。
- 任意の2評価期間×◎☆回数の銘柄別バブル図を追加。
- 未確定値を混ぜず、2期間比較では同じイベントで両方が確定した実績だけを利用。
- 保存済み履歴だけで描画し、通常表示や軸変更で外部取得を行わないHarness契約を追加。


## v0.6.67

- J-Quants実データ更新ごとの銘柄マスターを日付別・不変ファイルとして保存。
- 既存のcertified curated runから、追加API取得なしで履歴銘柄マスターを復元。
- Walk-Forwardで評価日以前の最新マスターを選び、保存済み履歴に残る上場廃止相当銘柄を過去母集団へ復帰。
- 履歴未取得期間は現在マスターへフォールバックし、利用可否・日付・経過日数・カバレッジ・警告を監査列と画面へ表示。
- 履歴マスター変更をWalk-Forward結果キャッシュの無効化条件へ追加。


## v0.6.66

- 条件設定・候補・履歴・実績検証・Walk-Forward・個別銘柄の主要項目へ初心者向けホバー説明を追加。
- すべての分析表を共通描画し、各列名へ意味を示すホバー説明を追加。
- クリック可能な一覧の列見出しで、項目の意味と並び替え操作を同時に説明。
- 説明表示がスコア計算や外部データ取得を変更しないことをHarness契約へ追加。


## v0.6.65

- Walk-Forward結果をデータ・条件・設定・バージョン単位でローカル永続キャッシュ。
- 過去時点の定量特徴量を再利用し、条件変更時の全銘柄再計算を削減。
- 将来リターン検索を銘柄別NumPy索引と二分探索へ変更。
- 簡易・標準・詳細モード、評価期間選択、再計算指定を追加。
- 再現時点、基準日、処理段階の進捗を表示。
- キャッシュ無効化、索引結果一致、特徴量再利用の回帰テストを追加。
