...
- ◎☆後の期間別縦棒を、取引日数×平均リターン×プラス率×確定件数の推移バブル図へ置き換えました。
- 「条件別グラフの期間」の縦棒を、平均リターン×プラス率×確定件数の条件バブル図へ置き換えました。
- 円の色と大きさから、平均値だけでなく勝ちやすさとデータ量を同時に確認できます。
- v0.6.68の条件バブルとの重複を整理し、条件別期間選択の場所へ表示を統一しました。

### v0.6.68 履歴実績の多面的な分析グラフ

- 条件×10/20/30/60/90/180取引日の平均リターンをヒートマップで俯瞰できます。
- 条件ごとの平均リターン、プラス率、確定件数を3要素のバブル図で比較できます。
- 銘柄ごとの任意の2期間リターンと◎☆回数をバブル図で比較し、継続上昇・遅れて回復・短期上昇後の失速を見分けやすくしました。
- 保存済みローカル履歴だけを使い、表示や軸変更ではJ-Quants／Yahoo取得を行いません。

### v0.6.67 履歴銘柄マスターによる生存者バイアス対策

- J-Quants更新ごとの上場銘柄一覧を日付別に保存し、既存の認証済み実データrunからも履歴を復元します。
- Walk-Forwardは評価日以前で最新の銘柄マスターを使うため、現在は上場していない銘柄も保存済み履歴に存在すれば過去母集団へ含められます。
- 履歴がない期間だけ現在マスターへフォールバックし、利用したマスター日、経過日数、カバレッジ、警告を結果へ残します。
- 保存済みのどのマスターにも存在しない銘柄は復元できないため、生存者バイアスを軽減する実装であり完全排除ではありません。
...
- 「履歴・実績検証」の3画面に、表示中データから件数・評価構成・実績傾向・注意点を短い箇条書きで整理する「分析結果サマリ」を追加しました。外部AI/APIは呼びません。

### v0.6.59 実績検証の短中期評価期間を拡張

- ◎☆実績検証の評価期間を 10日・20日・30日・60日・90日・180日に拡張しました。
- Yahoo実績更新、平均リターン、確定件数、銘柄別サマリ、イベント詳細、条件別実績グラフを同じ期間セットで統一しました。
- 既存の30/90/180日履歴は維持し、新しい10/20/60日は保存済みYahoo日足または次回の明示更新で補完できます。
...
### Walk-Forward過去検証の高速化（v0.6.65）

「履歴・検証 > Walk-Forward検証」では、簡易・標準・詳細モードを選べます。
同じデータ・条件・設定の結果はローカルキャッシュから再表示し、条件変更時も
過去時点の定量特徴量を再利用します。処理中は再現時点と基準日を進捗表示します。
キャッシュ利用中もJ-Quants/Yahooへの追加アクセスは行いません。

### 分析項目のホバー説明（v0.6.66）

条件設定、候補一覧、履歴・実績検証、Walk-Forward、個別銘柄画面では、主要な入力項目や
集計値の説明アイコン、または表の列名にマウスを置くと、意味を簡単な言葉で確認できます。
説明表示は既存のスコア計算・候補抽出・外部データ取得には影響しません。
